...
export class RefundsControllers{
    ...
    async index(req: Request, res: Response){
        ...
        const refunds = await prisma.refunds.findMany({ skip, take: perPage, 
            where: { 
                user: { 
                    name: {
                            contains: name.trim()
        ...
        res.status(200).json({ "message": refunds.length > 0 ? refunds : "Não tem reembolsos", "pagination": page, perPage, totalRecords, totalPages: totalPages > 0 ? totalPages : 1 })
    }
